 ...
 * Lisible hors ligne (cache). La position (lat/lng) n'entre PAS dans la clé de cache : elle ne
 * change que le tri par distance, pas l'ensemble filtré → l'offline sert la même liste.
 */
export async function rechercherStructures(filtres: FiltresStructure = {}): Promise<Structure[]> {
  const { lat: _lat, lng: _lng, ...cle } = filtres;
  return lireAvecCache(`structures:${JSON.stringify(cle)}`, async () => {
    const { data } = await api.get<StructuresResponse>('/v1/structures', { params: filtres });
    return data.structures;
  });
}

/** F3.5 — Fiche détaillée d'une structure (services + dispos du jour). Lisible hors ligne. */
export async function getStructure(id: number): Promise<Structure> {
  return lireAvecCache(`structure:${id}`, async () => {
    const { data } = await api.get<{ structure: Structure }>(`/v1/structures/${id}`);
    return data.structure;
  });
}

/** F3.8 — Pharmacies de garde du jour, triées par proximité si une position est fournie. Lisible hors ligne. */
export async function getPharmaciesGarde(position?: Coordonnees): Promise<Structure[]> {
  return lireAvecCache('pharmacies-garde', async () => {
...
/** F3.9 — Avis VISIBLES d'une structure (lecture publique, les plus récents d'abord). Lisible hors ligne. */
export async function getAvisStructure(id: number): Promise<Avis[]> {
  return lireAvecCache(`avis:${id}`, async () => {
...
export async function getSignalementsStructure(id: number): Promise<SignalementPublic[]> {
  return lireAvecCache(`signalements:${id}`, async () => {
