...
@section('titre', 'Modifier le besoin · ' . $besoin->groupe_sanguin)

@section('content')
<div class="d-flex align-items-center gap-2 mb-4">
  <a href="{{ route('portail.don-sang.index') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i></a>
  <h1 class="h3 mb-0" style="color:var(--ms-blue-dark)">Modifier le besoin</h1>
</div>

<form method="POST" action="{{ route('portail.don-sang.update', $besoin) }}">
  @csrf @method('PUT')
  <div class="card border-0 shadow-sm mb-3">
    <div class="card-body">
      @include('portail.don-sang._form', ['besoin' => $besoin])
    </div>
  </div>
